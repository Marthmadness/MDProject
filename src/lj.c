#include "md.h"
void lj(double **state, int N, double L, double epsilon, double sigma, double *p){
	double pressure=0;
	for(int i = 0; i < N-1; i++){//loop over all pairs
		for(int j=i+1; j<N; j++){
			...
			double force = -24*epsilon/(rij2) * (2*(pow(sigma,12)/(pow(rij2,6))) - (pow(sigma,6)/pow(rij2,3)));
			//now update each component of the force
			for(int k=3; k<6; k++){
				state[i][k] += force*rij[k-3];
				state[j][k] += -force*rij[k-3];//newton's 3rd law
				pressure += force*rij[k-3]*rij[k-3];
			}
		}
	}
	*p = pressure/(3*L*L*L);
